...
public class RepositorioHistoricoAdhocMongoDB {
	MongoCollection<Historico> mongoCollection;

	public RepositorioHistoricoAdhocMongoDB() {
		mongoCollection = generico();
	}

	public MongoCollection<Historico> generico() {
		/**************************************
		 * MONGODB
		 ****************************************/
	...
	public void update(Historico entity) throws RepositorioException, EntidadNoEncontrada {
	...
	public void delete(Historico entity) throws RepositorioException, EntidadNoEncontrada {
		// Filtro para buscar el documento por su ID
		ObjectId objectId;
        objectId = new ObjectId(entity.getId());
	...
	public Historico getById(String id) throws RepositorioException, EntidadNoEncontrada {
		...
		Historico entity = mongoCollection.find(filter).first();
	...
	public List<Historico> getAll() throws RepositorioException {
		// Lista para almacenar los documentos
		List<Historico> entities = new ArrayList<>();
		// Obtener un cursor a todos los documentos
		MongoCursor<Historico> cursor = mongoCollection.find().iterator();
		try {
			// Iterar sobre el cursor para obtener cada documento
			while (cursor.hasNext()) { 
				Historico entity = cursor.next();
				entities.add(entity);
			}
		} finally {
	...
	public List<Historico> obtenerPorIdBicicleta(String idBicicleta) {
	    ...
	    MongoCollection<Historico> collection = generico();
